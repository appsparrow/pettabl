...
const TodayActivityCard = ({ activities }: TodayActivityCardProps) => {
          ...
          <div className="space-y-3">
            {activities.map((activity) => (
              <div key={activity.id} className="flex items-center gap-3 p-3 bg-gradient-to-r from-gray-50 to-slate-50 rounded-2xl">
                <div className="text-2xl">{getActivityEmoji(activity.activity_type)}</div>
                <div className="flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <Badge className={`${getTimePeriodColor(activity.time_period)} rounded-full capitalize`}>
                ...
                {activity.photo_url && (
                  <img src={activity.photo_url} alt="Activity" className="w-12 h-12 rounded-xl object-cover" />
                )}
              </div>
            ))}
          </div>
